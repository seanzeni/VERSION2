...
def test_data_loader_loads_excel_and_releases(tmp_path: Path) -> None:
    """Verifies data loader loads excel and releases."""
...
def test_missing_column_raises(tmp_path: Path) -> None:
    """Verifies missing column raises."""
    path=tmp_path/'inventory.xlsx'
    pd.DataFrame([{'Release':'REL1'}]).to_excel(path,index=False)
    with pytest.raises(Exception):
        DataLoader(path, REQUIRED_COLUMNS).load()

def test_filter_release_projects(tmp_path: Path) -> None:
    """Verifies filter release projects."""
    path=tmp_path/'inventory.xlsx'
